...
// Enhance book.html share buttons
document.addEventListener('DOMContentLoaded', () => {
  const shareBookBtn = document.getElementById('share-book');
  if (shareBookBtn) {
    shareBookBtn.addEventListener('click', () => {
      const bookTitle = document.querySelector('.book-title')?.textContent || 'The Wonders of Nature';
      const currentUrl = window.location.href;
      sharePage(
        bookTitle,
        `Check out this amazing book: "${bookTitle}" - An interactive reading experience!`,
        currentUrl
      );
    });
  }
